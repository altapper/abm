...
class Worker(Person):
    ...
    def adjustTimes(self):
        self.starttime = round(np.random.normal(9, 0.7)) * ticksToAnHour
        self.timeatwork = round(np.random.normal(8, 0.5)) * ticksToAnHour
        self.endtime = self.starttime + self.timeatwork
        self.setofftime = self.starttime - self.journeylength
...
class Camera(object):
    ...
    def findLineOfSight(self):
        ...
        try: 
            len(lineofsight[0])
            lineofsight = random.choice(lineofsight)
        except TypeError:
            pass
        except IndexError:
            pass
...
def runProgram(dfIn, lengthIn, widthIn):
    ...
    buildAgents(dfIn, lengthIn, widthIn)
    buildCameras()
    runAgents()
